    ...
    >
      <b-container class="align-bottom" :style="this.windowsize"  >
          <div style="text-align: center;">
               <img :src="require('../../assets/Instructions/Questions TF/Q12a.png')" v-bind:style="{ maxWidth: '70%', height: 'auto', marginTop: '0%', transform: 'translate(-50%, 0)', position: 'absolute', opacity: this.opacity_a, transition: 'opacity 0.5s' }"/>
               <img :src="require('../../assets/Instructions/Questions TF/Q12b.png')" v-bind:style="{ maxWidth: '70%', height: 'auto', marginTop: '0%', transform: 'translate(-50%, 0)', position: 'absolute', opacity: this.opacity_b, transition: 'opacity 0.5s' }"/>
            </div>
              
      </b-container>

        <b-button @click="$bvModal.hide(`modal-center-instruction62`)" v-b-modal.modal-center-instruction61 variant="outline-primary" :disabled="allowprogress" size="lg">Back</b-button>
        <b-button @click="$bvModal.hide(`modal-center-instruction62`)" v-b-modal.modal-center-EndTF2 style="float: right;" variant="outline-primary" :disabled="allowprogress" size="lg">Next</b-button>
...
<script>
    export default {
        ...
        computed: {
            ...
            center_title() {
                return "True or False Question " + (Number(this.page_num) - 50).toString()
            },
        },
        mounted() {
        ...
        methods: {
            ...
            reset_animation_next() {
                this.$bvModal.hide("modal-center-instruction" + this.page_num)
                this.$bvModal.show("modal-center-instruction" + ((Number(this.page_num) + 7).toString()))
            },
            reveal_False() {
